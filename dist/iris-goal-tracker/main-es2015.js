...
let ApiService = class ApiService {
    ...
    get(endpoint, params, reqOpts) {
        ...
        return this.http.get(`${this.url}/${endpoint}`, reqOpts)
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["catchError"])(err => this._handleError(err)));
    }
...
let DashboardComponent = class DashboardComponent {
    constructor(user, utils, renderer) {
        this.user = user;
        this.utils = utils;
    ...
    createTodo(todoData) {
        this.user.createTodo(this.goal._id, todoData).subscribe((res) => {
    ...
    deleteTodo(todo) {
        this.user.removeTodo(todo._id).subscribe((res) => {
            const index = this.goal.todos.findIndex((todoObj) => todoObj._id === todo._id);
    ...
    loadGoals() {
        this.user.getGoals()
            .subscribe((res) => {
            this.goals = res.data.map((goal) => this.formatGoal(goal)).sort(this.sortLatest);
        }, err => {
            this.utils.showToast({ title: err, type: 'error' });
        });
    }
    loadSingleGoal(goal) {
        this.user.getSingleGoal(goal._id).subscribe((res) => {
    ...
    deleteGoal(goal) {
        swal.fire({
            ...
            if (result.value) {
                this.user.removeGoal(goal._id).subscribe((res) => {
    ...
    editGoal(goal) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            const { value: formValues } = yield swal.fire({
                title: 'Edit goal',
                html: `
        <div class="form-group">
        ...
        `,
            ...
            if (formValues) {
                this.user.updateGoal(goal._id, formValues).subscribe((res) => {
    ...
    editUser() {
        ...
        <div class="form-group">
          ...
          id="user-email-input" placeholder="someone@example.com" autocomplete="no">
            ...
            if (formValues) {
                this.user.updateUser(user._id, formValues).subscribe((res) => {
    ...
    markTodo(todo) {
        this.user.markTodo(todo._id, !todo.isComplete).subscribe((res) => {
            this.utils.showToast({ title: `Marked to-do as ${todo.isComplete ? 'undone 😞' : 'done 😊'}`, type: 'success' });
            this.goal.todos = this.goal.todos.map((todoObj) => (todoObj._id === res.data._id) ? res.data : todoObj);
            const meta = {
                totalTodos: { value: this.goal.todos.length },
                completedTodos: { value: this.goal.todos.filter((todoObj) => todoObj.isComplete).length }
            };
            this.goal.meta = meta;
    ...
    parseScheduleDate(scheduleDate) {
...
let LoginComponent = class LoginComponent {
    ...
    onSubmit(userData) {
        this.loading = true;
        this.user.login(userData).subscribe((res) => {
...
let RegisterComponent = class RegisterComponent {
    ...
    onSubmit(userData) {
        this.loading = true;
        this.user.register(userData).subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            this.signupForm.reset();
            this.utils.showToast({ title: 'Successfully signed up!', type: 'success' });
            const { value: token } = yield swal.fire({
            ...
            if (token) {
                this.user.verify(token).subscribe((user) => {
                    this.loading = false;
...
let UserService = class UserService {
    ...
    saveUser(user, triggerLoggedIn = false) {
        if (triggerLoggedIn) {
